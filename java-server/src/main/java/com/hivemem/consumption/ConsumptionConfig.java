...
public class ConsumptionConfig {
     ...
     * The Spring scheduler pool is sized &gt; 1 (spring.task.scheduling.pool.size) so an inline
     * caller-runs batch occupies only one scheduler thread and cannot starve every other
     * scheduled job while it OCRs.
     */
